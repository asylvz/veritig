...
void Validate::run_mapping(parameters& params)
{
	...
	minimap_cmd = "minimap2 -cx " + shell_quote(params.minimap_preset) + " -t " + threads_str + " --paf-no-hit " + shell_quote(params.haplo1_assembly_path) + " " + shell_quote(params.fasta) + " --secondary=no > " + shell_quote(this->paf_H1_sv_path) + " 2>/dev/null";
	run_command(minimap_cmd, this->paf_H1_sv_path);
	std::cerr << " done\n";

	std::cerr << "  Mapping SVs to H2...";
	minimap_cmd = "minimap2 -cx " + shell_quote(params.minimap_preset) + " -t " + threads_str + " --paf-no-hit " + shell_quote(params.haplo2_assembly_path) + " " + shell_quote(params.fasta) + " --secondary=no > " + shell_quote(this->paf_H2_sv_path) + " 2>/dev/null";
